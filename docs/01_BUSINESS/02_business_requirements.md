...
### Sets (S)
- **S001** – `User` creates a `set`, providing a required title, optionally a description and tags, and chooses the type: `standard` or `language`.
- **S002** – User edits a `set` (title, description, tags).
- **S003** – User deletes a `set` completely.
- **S004** – User copies a `public set` to their `space`; the copy becomes a fully independent `set` owned by the `user`.
- **S005** – The system limits the number of `flashcards` in a set to a maximum of 200.
